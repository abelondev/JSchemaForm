...
internal class Field {
        ...
        get {
            if let type = formItem.type {
                ...
                if let type = schema.type {
                    return Field.formType(type);
                }
                else {
                    return Field.formType(SchemaType.String);
                }
            }
        }
    }
    ...
    internal static func fieldList(properties:[String:JSchema], formItems:[JFormItem]?, requiredItems:[String]?) throws -> [Field] {
        ...
        if let formItems = formItems {
            for formItem in formItems {
                if formItem.isAll() {
                    appendAllFormItems(properties, fields: &fields);
                }
                else {
                    if let key = formItem.key, let schema = properties[key] {
                        ...
                        throw FormBuilderError.Missed("Can't find field '\(formItem.key!)' in properties")
                    }
                }
            }
        } else {
        ...
        //set required flags
        if let requiredItems = requiredItems where requiredItems.count > 0 {
            ...
            for requiredKey in requiredItems {
                if let found = fieldMap[requiredKey] {
                    ...
                    throw FormBuilderError.Missed("Can't find required field '\(requiredKey)'")
                }
            }
        }
        return fields;
    ...
    static func appendAllFormItems(properties:[String:JSchema], inout fields:[Field]) {
